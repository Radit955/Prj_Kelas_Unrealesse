...
@section('title', 'Edit Seksi Kegiatan')
@section('content')
<div class="max-w-3xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Edit Seksi</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Perbarui pengaturan seksi dan anggotanya.</p>
        </div>
    </div>

    <div class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm p-6">
        <form action="{{ route('admin.seksi.update', $seksi) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Nama Seksi</label>
                <input type="text" name="name" value="{{ old('name', $seksi->name) }}" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Kategori</label>
                <input type="text" name="category" value="{{ old('category', $seksi->category) }}" class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30">{{ old('description', $seksi->description) }}</textarea>
            </div>
            <div>
                ...
                <select name="ketua_id" class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                    ...
                    <option value="{{ $student->id }}" {{ old('ketua_id', $seksi->ketua_id) == $student->id ? 'selected' : '' }}>{{ $student->user->name ?? '—' }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="member_ids[]" multiple class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                    @foreach($students as $student)
                    <option value="{{ $student->id }}" {{ in_array($student->id, old('member_ids', $seksi->members->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $student->user->name ?? '—' }}</option>
                    @endforeach
                </select>
                <p class="mt-2 text-xs text-slate-400">Tekan Ctrl/Cmd untuk memilih beberapa siswa.</p>
            </div>
