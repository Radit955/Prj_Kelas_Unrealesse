...
<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Edit Pengguna</h1>
    </div>
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.users.update', $user) }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div x-data="{ preview: null }" class="flex items-center gap-4">
                <div class="w-16 h-16 rounded-full overflow-hidden border-2 border-slate-200 dark:border-slate-600 flex-shrink-0 bg-blue-600 flex items-center justify-center">
                    ...
                    @else
                    <span x-show="!preview" class="text-white font-bold text-xl">{{ strtoupper(substr($user->name,0,2)) }}</span>
                    @endif
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Ganti Foto</label>
            ...
            <div>
                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Bio</label>
                <textarea name="bio" rows="2" class="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg px-3 py-2.5 text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30 resize-none">{{ old('bio', $user->bio) }}</textarea>
            </div>
            <div class="border-t border-slate-100 dark:border-slate-700 pt-3">
                <p class="text-xs text-slate-500 dark:text-slate-400 mb-3 font-medium">Ganti Password (kosongkan jika tidak ingin mengubah)</p>
                <div class="grid grid-cols-2 gap-4">
                    <input type="password" name="password" placeholder="Password baru (min 8)"
                           class="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg px-3 py-2.5 text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
