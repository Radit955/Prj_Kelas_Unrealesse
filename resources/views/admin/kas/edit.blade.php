...
<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Edit Transaksi Kas</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Perbarui detail transaksi kas.</p>
        </div>
    </div>

    <div class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm p-6">
        <form action="{{ route('admin.kas.update', $kas) }}" method="POST" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select name="student_id" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                    ...
                    <option value="{{ $student->id }}" {{ old('student_id', $kas->student_id) == $student->id ? 'selected' : '' }}>{{ $student->user?->name }} ({{ $student->nis }})</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="amount" value="{{ old('amount', $kas->amount) }}" min="1" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
                    @error('amount')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Minggu</label>
                    <input type="number" name="week" value="{{ old('week', $kas->week) }}" min="1" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
            ...
            <div>
                ...
                <select name="method" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                    <option value="qris" {{ old('method', $kas->method) === 'qris' ? 'selected' : '' }}>QRIS</option>
                    <option value="cash" {{ old('method', $kas->method) === 'cash' ? 'selected' : '' }}>Cash</option>
                </select>
            ...
            <div>
                ...
                <select name="status" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                    <option value="paid" {{ old('status', $kas->status) === 'paid' ? 'selected' : '' }}>Paid</option>
                    <option value="unpaid" {{ old('status', $kas->status) === 'unpaid' ? 'selected' : '' }}>Unpaid</option>
                </select>
